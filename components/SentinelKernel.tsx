...
import { Cpu, Lock, CloudOff, Zap, ShieldAlert, Wifi, Activity, Terminal } from "lucide-react";
...
const features = [
  {
    ...
    desc: "Seamless Edge-to-Sync processing ensures data sovereignty while leveraging low-latency distributed computing.",
  },
  {
    icon: Lock,
    title: "Mathematically Secured",
    desc: "Primary safety logic is isolated from the UI layer, preventing interface glitches from impacting critical sensors.",
  },
  {
    icon: Zap,
    title: "Auditable Sovereign Stack",
    desc: "A fully transparent and independent runtime pipeline for long-term hardware independence.",
  },
];
...
export const SentinelKernel = () => {
  ...
  useGSAP(() => {
    // Initial state set via CSS or GSAP to ensure no FOUC but guaranteed visibility
  ...
  // Simulate a failover and telemetry
  useEffect(() => {
    ...
    const telemetryInterval = setInterval(() => {
      if (!isCrashed) {
        setActiveNodes(prev => Math.max(18, Math.min(32, prev + (Math.random() > 0.5 ? 1 : -1))));
      }
    }, 2000);
        ...
        <div className="space-y-12 relative z-10">
          <div className="feature-row space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-full">
               ...
               <span className="text-[10px] font-mono text-amber-500 font-bold uppercase tracking-widest">Sovereign Architecture</span>
            </div>
            ...
            <p className="text-gray-400 text-lg leading-relaxed max-w-xl font-medium">
              A bespoke spatial operating system designed for zero-latency. We mathematically separate the <span className="text-amber-500">Visual Pipeline</span> from the <span className="text-white">Security Foundation</span> to ensure protection is Always-On.
            </p>
          </div>
        ...
        <div className="feature-row relative h-[500px] md:h-[650px] w-full bg-black border border-white/5 rounded-[2rem] md:rounded-[3rem] overflow-hidden flex flex-col items-center justify-between p-8 md:p-12 group">
          ...
          <div className="w-full flex justify-between items-start z-10">
             <div className="space-y-1">
                <div className="text-[10px] font-mono text-zinc-500 uppercase">System_Load</div>
             ...
             <div className="text-right">
                <div className="text-[10px] font-mono text-zinc-500 uppercase">Active_Nodes</div>
                <div className="text-lg font-black text-amber-500">{isCrashed ? "00" : activeNodes}</div>
             </div>
          </div>
          ...
          <div className={`relative z-10 text-center space-y-6 md:space-y-8 transition-all duration-700 ${isCrashed ? 'opacity-20 grayscale scale-95 blur-sm' : 'opacity-100'}`}>
            <div className="w-32 h-32 md:w-48 md:h-48 mx-auto border-2 border-amber-500/50 rounded-full flex items-center justify-center shadow-[0_0_80px_rgba(255,176,0,0.1)] relative">
              ...
              <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-3 py-1 bg-amber-500 text-black text-[7px] md:text-[8px] font-black uppercase tracking-widest">SOVEREIGN CORE</div>
          ...
          <div className={`absolute inset-0 z-20 flex flex-col items-center justify-center transition-opacity duration-300 p-4 text-center ${isCrashed ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
             ...
             <div className="mt-4 text-[7px] md:text-[8px] font-mono text-red-500/50">PRIMARY_SAFETY_NODE_UNAFFECTED...</div>
          </div>
          ...
          <div className="relative z-30 w-full">
             <div className={`p-4 md:p-6 rounded-xl md:rounded-2xl border transition-all duration-500 flex items-center justify-between ${isCrashed ? 'bg-red-500/10 border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.2)]' : 'bg-white/5 border-white/10'}`}>
                <div className="flex items-center gap-3 md:gap-4">
                   ...
                   <div>
                      <div className={`text-[7px] md:text-[8px] font-black uppercase tracking-widest ${isCrashed ? 'text-red-500' : 'text-zinc-500'}`}>Security Foundation</div>
                ...
                <div className="flex gap-2">
                   ...
                   <div className="text-[8px] md:text-[10px] font-mono text-white/40 font-bold uppercase hidden xs:block">SECURE</div>
                </div>
             </div>
          </div>

          <div className="absolute top-1/2 -left-20 -rotate-90 font-mono text-[8px] text-zinc-800 tracking-[1em] uppercase hidden lg:block">
             Sentinel_OS_Bespoke_Architecture
          </div>
